    ...
    <section id="gallery" className="py-5 bg-light position-relative">
      <Container className="py-4">
        <div className="text-center mb-5" data-aos="fade-up">
          <span className="text-primary fw-bold text-uppercase tracking-wider">VISUAL TOUR</span>
          <h2 className="display-5 fw-bold text-dark mt-1">Store & Products Gallery</h2>
          <p className="text-muted fs-6">Explore our inventory of textbooks, diagnostic instruments, and clinical attire.</p>
        </div>

        <Row className="g-4">
          {galleryImages.map((img, idx) => (
            <Col key={idx} md={idx < 2 ? 6 : 4} data-aos="fade-up" data-aos-delay={idx * 100}>
              <div className="glass-card overflow-hidden position-relative group" style={{ height: '280px' }}>
                <img
                  src={img.url}
                  alt={img.title}
                ...
                <div className="position-absolute bottom-0 start-0 end-0 p-3 bg-dark bg-opacity-60 text-white backdrop-blur">
                  <small className="text-info text-uppercase fw-semibold d-block mb-1" style={{ fontSize: '11px' }}>
                    {img.category}
                  </small>
